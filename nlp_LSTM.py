...
class Data :
  ...
  def __init__(self,fname='texts/english') :
    ...
    hot_X = hot_X.reshape(-1, 1, hot_X.shape[1])
    self.enc_X = enc_X
    self.enc_y = enc_y
    self.X=X
    ...
    for ys in Y :
      ...
      ms.append(m[0])
    hot_y=np.array(ms)
    hot_y = hot_y.reshape(-1, 1, hot_y.shape[1])
...
class Trainer(Data) :
  ...
  def __init__(self,fname='texts/english'):
    ...
    model.add(LSTM(128, input_shape=(1,self.hot_X.shape[2]), activation='relu', return_sequences=True,))
    model.add(LSTM(self.hot_y.shape[2],input_shape=(1,self.hot_X.shape[2]), activation='sigmoid', return_sequences=True,))
    model.summary()
...
def ntest() :
  ...
  i.ask("What did google researchers introduce?")
  i.ask(text="Why building explainable NLP systems is important?")
  print("\n")
  print("NEURAL NET'S ANSWERS:\n")
  i.query("What did google researchers introduce?")
  i.query(text="hy building explainable NLP systems is important?")
